...
<template>
  <div class="p-3 sm:p-4 md:p-6 lg:p-8 w-full bg-gray-50 min-h-screen">
    <!-- Header -->
    <header class="mb-4 sm:mb-6 md:mb-8 border-b border-purple-200 pb-3 sm:pb-4 flex flex-col sm:flex-row items-center gap-2 sm:gap-4">
      <i class="fas fa-calendar-alt text-2xl sm:text-3xl md:text-4xl text-purple-600"></i>
      <h1 class="text-xl sm:text-2xl md:text-3xl font-extrabold text-gray-800 tracking-tight text-center sm:text-left">
    ...
    <div class="bg-white shadow-lg rounded-xl p-4 sm:p-5 md:p-6 mb-4 sm:mb-6 md:mb-8 border border-gray-100">
      <h2 class="text-base sm:text-lg font-semibold text-gray-700 mb-3 sm:mb-4 flex items-center">
        <i class="fas fa-filter text-purple-500 mr-2"></i> Filtres
      </h2>
      <div class="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
        <div class="relative">
          <select
            v-model="filterStatus"
            class="appearance-none block w-full px-3 sm:px-4 py-2 sm:py-3 text-sm sm:text-base border border-gray-300 rounded-lg bg-gray-50 focus:ring-2 focus:ring-purple-500 focus:border-purple-500 transition duration-150 text-gray-700"
          >
        ...
        <div class="relative">
          <input
            v-model="filterDate"
            type="date"
            class="block w-full px-3 sm:px-4 py-2 sm:py-3 text-sm sm:text-base border border-gray-300 rounded-lg bg-gray-50 focus:ring-2 focus:ring-purple-500 focus:border-purple-500 transition duration-150 text-gray-700"
          />
        </div>
      </div>
    </div>
    ...
    <!-- Loading -->
    <div v-if="isLoading" class="flex flex-col items-center justify-center gap-3 sm:gap-4 text-purple-600 py-10 sm:py-12 md:py-16 bg-white rounded-xl shadow-lg">
    ...
    <div v-else>
      ...
      <div v-if="filteredAppointments.length > 0" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4 md:gap-6">
        <div
          ...
          class="bg-white border-l-4 p-4 sm:p-5 rounded-xl shadow-md hover:shadow-xl transition-all duration-300 cursor-pointer flex flex-col justify-between"
          :class="{
        ...
        >
          ...
          <div class="space-y-1 sm:space-y-2 mb-3 sm:mb-4">
            <p class="text-xs sm:text-sm text-gray-600 flex items-center">
              <i class="fas fa-calendar-check text-purple-400 w-4 sm:w-5 mr-2 flex-shrink-0"></i>
              <span class="font-medium text-gray-800">{{ formatDateDayMonthYear(appt.scheduled_at) }}</span>
            </p>
            <p class="text-xs sm:text-sm text-gray-600 flex items-center">
              <i class="fas fa-passport text-purple-400 w-4 sm:w-5 mr-2 flex-shrink-0"></i>
              <span class="truncate">Demande #<span class="font-medium text-gray-800">{{ appt.visa_request_id.substring(0, 8) }}...</span></span>
            </p>
          </div>

          <button class="text-xs sm:text-sm font-semibold text-purple-600 hover:text-purple-700 transition duration-150 self-end mt-2 flex items-center">
      ...
      <div v-if="filteredAppointments.length > 0" class="flex flex-col sm:flex-row justify-center items-center gap-2 sm:gap-0 mt-6 sm:mt-8">
        <button
          ...
          class="w-full sm:w-auto px-3 sm:px-4 py-2 rounded-lg sm:rounded-l-lg sm:rounded-r-none border sm:border-r-0 bg-white text-purple-600 font-semibold hover:bg-purple-50 transition duration-150 disabled:opacity-50 disabled:cursor-not-allowed shadow-sm text-sm"
        >
        ...
        <button
          ...
          class="w-full sm:w-auto px-3 sm:px-4 py-2 rounded-lg sm:rounded-r-lg sm:rounded-l-none border bg-white text-purple-600 font-semibold hover:bg-purple-50 transition duration-150 disabled:opacity-50 disabled:cursor-not-allowed shadow-sm text-sm"
        >
