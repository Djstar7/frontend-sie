...
<template>
  <div class="p-4 md:p-8 w-full bg-gray-50 min-h-screen">
    ...
    <header class="mb-8 border-b border-blue-200 pb-4 flex justify-center items-center">
      <i class="fas fa-calendar-alt text-4xl text-blue-600 mr-4"></i>
      <h1 class="text-3xl font-extrabold text-blue-600 tracking-tight">Mes Rendez-vous</h1>
    </header>

    <div class="bg-white shadow-lg rounded-xl p-6 mb-8 border border-gray-100">
        <h2 class="text-lg font-semibold text-gray-700 mb-4 flex items-center">
            <i class="fas fa-filter text-blue-500 mr-2"></i> Options de Filtrage
        </h2>
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div class="relative">
                <select
                    v-model="filterStatus"
                    class="appearance-none block w-full px-4 py-3 border border-gray-300 rounded-lg bg-gray-50 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition duration-150 text-gray-700"
                >
            ...
            <div class="relative">
                <input
                    v-model="filterDate"
                    type="date"
                    class="block w-full px-4 py-3 border border-gray-300 rounded-lg bg-gray-50 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition duration-150 text-gray-700"
                />
            </div>
        </div>
    </div>

    <div v-if="isLoading" class="flex flex-col items-center justify-center gap-4 text-blue-600 py-16 bg-white rounded-xl shadow-lg">
    ...
    <div v-else>
      <div v-if="filteredAppointments.length > 0" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        <div
          ...
          class="bg-white border-l-4 p-5 rounded-xl shadow-md hover:shadow-lg transition-all duration-300 cursor-pointer flex flex-col justify-between"
          :class="{
        ...
        >
            ...
            <div class="space-y-2 mb-4">
                ...
                <p class="text-sm text-gray-600 flex items-center">
                    <i class="fas fa-passport text-blue-400 w-5 mr-2"></i>
                    Demande #<span class="font-medium text-gray-800 ml-1">{{ appt.visa_request_id.substring(0, 8) }}...</span>
                </p>
            </div>

            <button class="text-sm font-semibold text-blue-600 hover:text-blue-700 transition duration-150 self-end mt-2 flex items-center">
      ...
      <div v-if="filteredAppointments.length > 0" class="flex justify-center items-center mt-8">
        <button
          ...
          class="px-4 py-2 rounded-l-lg border border-r-0 bg-white text-blue-600 font-semibold hover:bg-blue-50 transition duration-150 disabled:opacity-50 disabled:cursor-not-allowed shadow-sm"
        >
        ...
        <button
          ...
          class="px-4 py-2 rounded-r-lg border bg-white text-blue-600 font-semibold hover:bg-blue-50 transition duration-150 disabled:opacity-50 disabled:cursor-not-allowed shadow-sm"
        >
