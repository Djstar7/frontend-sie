...
<template>
  <div class="min-h-screen">
      ...
      <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8 p-6 bg-white rounded-xl shadow-lg border border-purple-100">
        <h1 class="text-3xl md:text-4xl font-extrabold text-gray-800 flex items-center gap-3">
          <i class="fas fa-passport text-purple-600 text-4xl"></i> Mes Demandes
        </h1>
        <router-link
          :to="{ name: 'custom.visarequest.create' }"
          class="mt-4 sm:mt-0 px-6 py-2 bg-purple-600 hover:bg-purple-700 text-white font-semibold rounded-lg shadow-md transition-all duration-200 flex items-center gap-2"
        >
      ...
      <div v-else-if="visaRequests.length === 0" class="text-center py-20 bg-white rounded-xl shadow-md border border-gray-100">
        <i class="fas fa-search-minus text-6xl text-gray-300 mb-4"></i>
        <p class="text-xl font-semibold text-gray-600">
          Vous n'avez pas encore de demande de visa en cours.
        </p>
        <router-link
          :to="{ name: 'custom.visarequest.create' }"
          class="mt-4 inline-flex items-center gap-2 px-6 py-3 bg-blue-500 text-white font-medium rounded-lg hover:bg-blue-600 transition"
        >
          <i class="fas fa-arrow-right"></i> Démarrer une nouvelle demande
        </router-link>
      </div>
        ...
        >
          ...
          <div class="flex flex-wrap gap-2 mt-auto pt-3 border-t border-gray-100">
            ...
            <button
              ...
              class="flex-1 min-w-[100px] bg-blue-600 hover:bg-blue-700 text-white font-semibold text-sm py-2 rounded-lg flex items-center justify-center gap-2 transition"
            >
              <i class="fa-solid fa-cloud-upload-alt"></i> Uploader
            </button>

            <button
              v-if="request.status === 'pending'"
              @click="openPay(request)"
              class="flex-1 min-w-[100px] bg-green-600 hover:bg-green-700 text-white font-semibold text-sm py-2 rounded-lg flex items-center justify-center gap-2 transition"
            >
            ...
            <button
              ...
              class="flex-1 min-w-[100px] bg-transparent text-red-500 border border-red-500 hover:bg-red-50 text-sm py-2 rounded-lg flex items-center justify-center gap-2 transition"
